...
import meetup = require("./Meetup"); 


function meetupPlanner(meetupSchedule : scheduler.Scheduler, year : number, month : number, dayOfWeek : string, schedule : string) {
   ...
   let result = meetupSchedule.meetupDay(year, month, dayOfWeek, schedule); 
   let day = weekday[result.getDay()];
